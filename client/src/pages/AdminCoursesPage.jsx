...
const AddCourseModal = ({ isOpen, onClose, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-lg">
                <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Tambah Mata Kuliah</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="label">Kode MK</label>
...
export default function AdminCoursesPage() {
    ...
    const handleAddCourse = async (courseData) => {
        try {
            ...
            Swal.fire(getSwalConfig({ title: 'Berhasil!', text: 'Mata kuliah berhasil ditambahkan', icon: 'success' }));
            loadCourses();
        } catch (error) {
            console.error('Failed to add course:', error);
            Swal.fire(getSwalConfig({ title: 'Error', text: error.response?.data?.message || 'Gagal menambahkan mata kuliah', icon: 'error' }));
        }
    };
