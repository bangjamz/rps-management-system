  ...
  try {
    ...
    try {
      if (globalSettings?.logo_path) {
        const logoPath = `/${globalSettings.logo_path}?t=${new Date().getTime()}`;
    ...
    doc.text("RENCANA PEMBELAJARAN SEMESTER", pageWidth / 2, 15, {
      align: "center",
    });

    // Dynamic Prodi/Fakultas from MK
    const prodiName =
      course?.prodi?.nama || rpsData?.mata_kuliah?.prodi?.nama || "-";
    const fakultasName =
      course?.prodi?.fakultas?.nama ||
      rpsData?.mata_kuliah?.prodi?.fakultas?.nama ||
      "-";
    const ptName =
      globalSettings?.nama_pt || "INSTITUT TEKNOLOGI DAN KESEHATAN MAHARDIKA";

    doc.setFontSize(11);
    doc.text(`PROGRAM STUDI ${prodiName.toUpperCase()}`, pageWidth / 2, 21, {
      align: "center",
    });
    doc.setFontSize(10);
    doc.text(`FAKULTAS ${fakultasName.toUpperCase()}`, pageWidth / 2, 27, {
      align: "center",
    });
    ...
    autoTable(doc, {
      ...
      body: [
        [
          "Nama MK",
          course?.nama_mk || rpsData?.mata_kuliah?.nama_mk || "-",
          "Kode MK",
          course?.kode_mk || rpsData?.mata_kuliah?.kode_mk || "-",
          "Rumpun MK",
          rpsData?.rumpun_mk || "-",
        ],
        [
          ...
          sksText,
          "Semester",
          `${rpsData?.semester || course?.semester || "-"}`,
          "Tgl Penyusunan",
          rpsData?.updated_at
            ? new Date(rpsData.updated_at).toLocaleDateString("id-ID")
            : "-",
        ],
      ],
      theme: "grid",
    ...
    if (rpsData?.cpl?.length > 0) {
      autoTable(doc, {
        ...
        head: [
          [
            {
              content: "CAPAIAN PEMBELAJARAN LULUSAN (CPL)",
              colSpan: 2,
              styles: { halign: "center", fillColor: [180, 180, 180] },
            },
          ],
        ],
    ...
    if (rpsData?.cpmk?.length > 0) {
      autoTable(doc, {
        ...
        head: [
          [
            {
              content: "CAPAIAN PEMBELAJARAN MATA KULIAH (CPMK)",
              colSpan: 2,
              styles: { halign: "center", fillColor: [180, 180, 180] },
            },
          ],
    ...
    autoTable(doc, {
      ...
      body: [
        [
          ...
          rpsData?.deskripsi_mk || "-",
        ],
        [
          {
            content: "MK Prasyarat",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          rpsData?.mk_syarat || "-",
        ],
    ...
    autoTable(doc, {
      ...
      head: [
        [
          {
            content: "PUSTAKA & MEDIA PEMBELAJARAN",
            colSpan: 2,
            styles: { halign: "center", fillColor: [200, 200, 200] },
          },
        ],
      ...
      body: [
        [
          {
            content: "Pustaka Utama",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          rpsData?.pustaka_utama || "-",
        ],
        [
          {
            content: "Pustaka Pendukung",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          rpsData?.pustaka_pendukung || "-",
        ],
        [
          {
            content: "Media Software",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          rpsData?.media_software || "-",
        ],
        [
          {
            content: "Media Hardware",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          rpsData?.media_hardware || "-",
        ],
      ],
      theme: "grid",
      styles: { fontSize: 8 },
    ...
    const dosenList = Array.isArray(rawDosenList)
      ? rawDosenList
          .map((d) => (typeof d === "string" ? d : d.nama || d.nama_lengkap))
          .join(", ")
      : rawDosenList;
    ...
    autoTable(doc, {
      ...
      head: [
        [
          {
            content: "DOSEN PENGAMPU & KETENTUAN",
            colSpan: 2,
            styles: { halign: "center", fillColor: [200, 200, 200] },
          },
        ],
      ...
      body: [
        [
          {
            content: "Dosen Pengampu",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          dosenList || rpsData?.dosen?.nama_lengkap || "-",
        ],
        [
          {
            content: "Ambang Batas Kelulusan MK",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          rpsData?.ambang_batas_mk || "55%",
        ],
        [
          {
            content: "Ambang Batas Kehadiran",
            styles: { fontStyle: "bold", fillColor: [230, 230, 230] },
          },
          rpsData?.ambang_batas_mhs || "75%",
        ],
      ],
      theme: "grid",
      styles: { fontSize: 8 },
    ...
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.text("RENCANA PEMBELAJARAN MINGGUAN", pageWidth / 2, 15, {
      align: "center",
    });
    ...
    const sortedPertemuan = [...pertemuanData].sort(
      (a, b) => (a.minggu_ke || 0) - (b.minggu_ke || 0),
    );
    let skipUntil = 0;
    ...
    sortedPertemuan.forEach((pertemuan) => {
      ...
      if (pertemuan.is_uts || (i === 8 && pertemuan.materi?.includes("UTS"))) {
        tableBody.push([
          ...
          {
            content: "EVALUASI TENGAH SEMESTER (UTS)",
            colSpan: 4,
            styles: { halign: "center", fontStyle: "bold" },
          },
          {
            content: pertemuan.bobot_penilaian || "22",
      ...
      if (pertemuan.is_uas || (i === 16 && pertemuan.materi?.includes("UAS"))) {
        tableBody.push([
          ...
          {
            content: "EVALUASI AKHIR SEMESTER (UAS)",
            colSpan: 4,
            styles: { halign: "center", fontStyle: "bold" },
          },
          {
            content: pertemuan.bobot_penilaian || "26",
            styles: { halign: "center" },
          },
        ]);
        return;
      ...
      const namaLms = pertemuan.nama_lms
        ? String(pertemuan.nama_lms).trim()
        : "";
      const platform = pertemuan.link_meet_platform
        ? String(pertemuan.link_meet_platform).trim()
        : "";
      const link = pertemuan.link_daring
        ? String(pertemuan.link_daring).trim()
        : "";
      const penugasan = pertemuan.penugasan
        ? String(pertemuan.penugasan).trim()
        : "";
      ...
      if (
        pertemuan.teknik_penilaian &&
        Array.isArray(pertemuan.teknik_penilaian) &&
        pertemuan.teknik_penilaian.length > 0
      ) {
        kriteriaParts.push(`Teknik: ${pertemuan.teknik_penilaian.join(", ")}`);
      }

      const kriteriaSection = kriteriaParts.join("\n");
      const indikatorKriteriaText = [indikatorSection, kriteriaSection]
        .filter(Boolean)
        .join("\n\n");
      ...
      tableBody.push([
        ...
        pertemuan.materi || "-",
        {
          content: pertemuan.bobot_penilaian?.toString() || "0",
          styles: { halign: "center" },
        },
      ]);
    });
    ...
    if (metodePenilaianList.length > 0) {
      ...
      autoTable(doc, {
        ...
        head: [
          [
            {
              content: "METODE PENILAIAN & KRITERIA",
              styles: {
                halign: "center",
