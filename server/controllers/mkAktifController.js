import { MKAktif, MataKuliah, Prodi, AcademicYear, User } from '../models/index.js';
    ...
    try {
        ...
        const mkAktifs = await MKAktif.findAll({
            ...
            include: [
                ...
                { model: User, as: 'dosen_pengampu', attributes: ['id', 'nama_lengkap', 'email'] }
            ],
    ...
    try {
        ...
        const mkAktifs = await MKAktif.findAll({
            where: {
                ...
                ...(user.angkatan && {
                    angkatan_target: { [Op.contains]: [user.angkatan] }
                })
            },
            include: [
                { model: MataKuliah, as: 'mata_kuliah' },
...
export const upsertMKAktif = async (req, res) => {
    try {
        const { mata_kuliah_id, prodi_id, tahun_akademik_id, semester, angkatan_target, dosen_pengampu_id, is_active } = req.body;
...
export const bulkAssignAngkatan = async (req, res) => {
    try {
        ...
        if (!mk_aktif_ids || !Array.isArray(mk_aktif_ids) || !angkatan) {
            return res.status(400).json({ message: 'mk_aktif_ids array and angkatan required' });
        }
        ...
        res.json({ message: `Assigned ${updated.length} courses to angkatan ${angkatan}` });
    } catch (error) {
