...
export const upsertProfile = async (req, res) => {
    try {
        const userId = req.user.id;
        const {
            profile_type, phone, address,
            nidn, jabatan_fungsional, pendidikan_terakhir, bidang_keahlian, homebase, status_kepegawaian, // Dosen
            nim, angkatan, status_mahasiswa // Mahasiswa
        } = req.body;
        ...
        if (profile) {
            ...
            if (profile_type === 'dosen') {
                ...
                if (pendidikan_terakhir) profile.pendidikan_terakhir = pendidikan_terakhir;
            ...
            profile = await UserProfile.create({
                user_id: userId,
                profile_type: req.user.role === 'dosen' ? 'dosen' : 'mahasiswa', // Default based on role
                phone, address,
                nidn, jabatan_fungsional, pendidikan_terakhir, bidang_keahlian, homebase, status_kepegawaian,
                nim, angkatan, status_mahasiswa
            });
        }
