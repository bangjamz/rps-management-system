...
import { seedInstitutCPL, seedFakultasCPL, seedProdiCPL, seedProfilLulusan } from './cplHierarchy.js';
...
const runSeeder = async () => {
    try {
        ...
        const { institusi, fakultasList, prodiList } = await seedOrganizationStructure();
        ...
        console.log('\n   📚 CPL Hierarchy:');
        console.log(`       - ${cplInstitut.length} Institut-level CPLs (mandatory for all)`);
        console.log(`       - ${cplFakultas.length} Fakultas-level CPLs (mandatory per faculty)`);
